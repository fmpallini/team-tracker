...
- **`src/core/`** — headless logic, no DOM construction:
  ...
  - `save-controller.ts` — save orchestration: auto-save interval from `prefs.autoSaveMin`, `saveNow()` (coalesces in-flight saves into trailing round), `flush()`, `runExclusive()` for non-save writers (e.g. password change) so two writers never race file handle. `backup-controller.ts` mirrors every successful save (and any password/format change) to second, user-picked file handle (`prefs.dailyBackupEnabled`/`backupHandleId`) — `.bck` sibling for corruption resilience, throttled to interval implied by `prefs.backupFrequency` (`'daily'` → 24h, `'hourly'` → 1h) except immediately after password change. The elapsed-time clock is seeded from the `.bck` file's own on-disk `lastModified` (via `getFile()`) the first time its handle is resolved each session — not hardcoded to "just reopened" — so the interval survives a reopen instead of writing on the very next save regardless of how recently the file was actually backed up; a freshly-picked, still-empty `.bck` (`size === 0`) is exempted so the very first real write isn't skipped.
